...
namespace GameFramework.Taurus
{
    ...
    public class GenerateILRuntimeAdapterWindow: EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Generate1", GUILayout.Width(80), GUILayout.Height(20)))
            {
                if (!isClickGenerateBtn)
                {
                    isClickGenerateBtn = true;
        ...
        private async Task CreateInterfaceAdapter(TypeDefinition type)
        {
            if (!type.IsInterface)
                return;
            ...
            using (var fs = File.Create(_out_path + "/" + adaptorName + ".cs"))
            {
                ...
                Debug.Log($"{type}：{classbody}");
                
                var sw = new StreamWriter(fs);
                await sw.WriteAsync(classbody);
                await sw.FlushAsync();
            }
        ...
        /// 生成
        /// </summary>
        private async Task GenerateClick()
        {
        ...
        private void LoadMainProjectOneAssembly(Stream pDllStream)
        {
             if (pDllStream == null)
            {
            ...
            try
            {
                var module = ModuleDefinition.ReadModule(pDllStream);
                ...
                Print($"----------------main scripts {pDllStream} assembly loaded" , ToolLogType.Normal);
            }
            catch (Exception ex)
            {
        ...
        private void LoadMainProjectAssemblyClick()
        {
            ...
            var dllNameArray = targetPath.Split(',');
            
            _adaptorSingleInterfaceDic.Clear();
            _adaptorDic.Clear();
            _delegateCovDic.Clear();
            
            foreach (string dllName in dllNameArray)
            {
                var path = $"{Application.dataPath}/../Library/ScriptAssemblies/{dllName}.dll";
                using (this.dllMS = new FileStream(path, FileMode.Open, FileAccess.Read))
                {
                    LoadMainProjectOneAssembly(this.dllMS);
                }
