...
{
    public static readonly string RootPath = $"{Application.dataPath}/Bundles/";
...
/*        var groupList = setting.groups;
        ...
        ///创建分组
        string loaclRoot = $"{RootPath}/Local";
        string remotedRoot = $"{RootPath}/Remote";
        ...
        foreach (var info in dirs)
        {
            ...
            string group_name = "Remote_" + info.Name;
            var group = setting.FindGroup(group_name);
            if (group == null)
            {
                group = setting.CreateGroup(group_name, false, false, false, new List<AddressableAssetGroupSchema> { setting.DefaultGroup.Schemas[0], setting.DefaultGroup.Schemas[1] });
            }
            AutoMarkRootAddress("Remote", info);
    ...
    {
        string path = local ? "Local" : "Remote";
        string root = $"{RootPath}/{path}/{name}";
        DirectoryInfo dir = new DirectoryInfo(root);
        DirectoryInfo[] dirs = dir.GetDirectories();
        string group_name = path + "_" + name;
        foreach (var info in dirs)
        {
            string assetPath = $"{root}/{info.Name}";
    ...
    private static void markFiles(string path, string name, DirectoryInfo dir, bool local = true)
    {
        ...
        if (files != null && files.Length > 0)
        {
            foreach (var file in files)
            {
                ...
                {
                    int index = file.Name.IndexOf(".", StringComparison.Ordinal);
                    string address = file.Name.Remove(index, file.Name.Length - index);
                    string group_name = local ? "Local_" + name : "Remote_" + name;
                    ...
                    {
                        allDirs = (path + "/" + dir.Name).Replace("Assets/AddressableAssets/Remote/" + name + "/", string.Empty).Split('/');
                    }
                    label.Add(name);
    ...
    private void SetParameters()
    {
        ...
        if (UnityEditor.PlayerSettings.companyName != "snakeyes")
            UnityEditor.PlayerSettings.companyName = "snakeyes";
        if (UnityEditor.PlayerSettings.productName != "et")
            UnityEditor.PlayerSettings.productName = "et";
        ...
        if (UnityEditor.PlayerSettings.applicationIdentifier != "com.snakeyes.et")
            UnityEditor.PlayerSettings.applicationIdentifier = "com.snakeyes.et";
    }


    /// <summary>
    /// 为资源标记分组，根据不同选项设置是否本地或远程
    /// 0 小包，所有资源存放资源服务器
    /// 1 分包 ，Local资源存本地，Remote资源存资源服务器
    /// 2 整包，所有资源存本地
    /// </summary>
