...
namespace ET.Client
{
    ...
    public static partial class FUIComponentSystem
    {
        ...
        public static void CollectFUIGComponentInfo(this FUIComponent self)
        {
            self.FUIGComponentInfoDic.Clear();
            var types = typeof(FUIComponent).Assembly.GetTypes();
            foreach (var type in types)
            {
                ...
                var _attrs = type.GetCustomAttributes(typeof(FUIGComponentAttribute), false);
                if (_attrs.Length > 0)
                {
                    if (_attrs[0] is FUIGComponentAttribute _attr)
                    {
                        self.FUIGComponentInfoDic.Add(type.FullName, _attr.mFUIGComponentInfo);
                    }
                }
            }
        }
