...
class UnityXMLHttpRequest extends XMLHttpRequestBase {
	...
	public $tick() {
		if (this._request) {
			...
			if (this._request.result == UnityEngine.Networking.UnityWebRequest.Result.ConnectionError || 
				this._request.result == UnityEngine.Networking.UnityWebRequest.Result.ProtocolError || this._request.isDone) {
				this.$finished_load();
	...
	private $finished_load() {
		...
		if (this._request.isDone 
			|| this._request.result == UnityEngine.Networking.UnityWebRequest.Result.ProtocolError) {
			this._internal_respons_headers = this._request.GetResponseHeaders();
