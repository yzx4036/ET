...
namespace ET
{
    ...
    public static class FUIHelper1
    {
        ...
        #region 创建FUI实例
        //
        // public static void Open(Action<FUI> callBack)
        // { 
        ...
        public static async ETTask<FUI1> OpenAsync<T>() where T : Entity, IAwake<FUI1>, new()
        {
            FUIInstArgs uiArgs = null;
            Type uiType = typeof (T);
            if (FUIInstArgsDict.TryGetValue(uiType, out uiArgs))
            {
                ...
                FUI1 fui = await _fuiComp.OpenAsync(uiArgs.UIPackageName, uiArgs.UIResName, uiArgs.FuiTypeHashCode);
                fui.AddComponent<T, FUI1>(fui);
                return fui;
            }
        ...
        {
            FUI1[] allFui = Game.Scene.GetComponent<FUIComponent>().GetAll();
