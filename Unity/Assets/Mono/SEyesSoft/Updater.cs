...
using UnityFS;
using UnityFS.Utils;
using Manifest = UnityFS.Manifest;
using UnityEngine;
...
namespace SEyesSoft
{
    public class Updater : MonoBehaviour, IAssetProviderListener, INetworkMonitorListener
    {
        ...
        private void Start()
        {
            ...
            _pools = new PrefabPools(new GameObject("_GameObjectPool"));
            gameObject.AddComponent<TaskInspector>();

            // 可用下载地址列表 (会依次重试, 次数超过地址数量时反复重试最后一个地址)
            // 适用于 CDN 部署还没有全部起作用时, 退化到直接文件服务器地址
            var urls = Helpers.URLs(
                // "http://localhost:8081/",
                "http://localhost:8080/"
            );
            ...
            ResourceManager.Initialize(new ResourceManagerArgs()
            {
                ...
                oninitialize = () =>
                {
                    Debug.Log(">>>>>>>>UnityFS 初始化 start");
                    ResourceManager.SetListener(this); // [可选] 监听事件
                },
                oncomplete = OnUnityFSInitComplete
            });
        }

        private void OnUnityFSInitComplete()
        {
            Debug.Log(">>>>>>>>UnityFS 初始化 完成！！！！！！！！！");
            var bundles = ResourceManager.GetInvalidatedBundles(Manifest.BundleLoad.Startup);
            ResourceManager.GetListener().OnStartupTask(bundles.ToArray());
        }
        ...
        private void OnGUI()
        {
            ...
            {
                Helpers.ReadSAManifest("test",
                    (manifest, fileEntry) => { Debug.LogFormat("Read SA Manifest: {0}", manifest.build); });
            }

            if (GUILayout.Button("Validate"))
            {
                ResourceManager.ValidateManifest(ResourceManager.urls,
                    result => { Debug.LogFormat("当前清单状态: {0}", result); });
            }
        }
        ...
        private void LoadGameScene()
        {
            ...
            var uScene = ResourceManager.LoadScene($"Assets/Scenes/{initScene}");
            Debug.Log("加载Init游戏场景");
            uScene.completed += scene =>
            {
                Debug.Log(">>>Init scene loaded<<<<<<<>>>>>>>>>>>>");
            };
        }
        
        private void OnAllBundleDownloadCompleted()
        {
            Debug.Log(">>>>>OnAllBundleDownloadCompleted");
            // UnityFS.ResourceManager.Instantiate("Assets/Bundles/Cube.prefab").DestroyAfter(10.0f);
            LoadGameScene();
            // // 在 zip 包中的文件可以异步加载
        ...
        public void OnStartupTask(UnityFS.Manifest.BundleInfo[] bundles)
        {
            ...
            var isNeedDownload = bundles.Length > 0;
            if (isNeedDownload)
            {
                var isDownloadNow = true;
                if (isDownloadNow)
                {
                    ResourceManager.EnsureBundles(bundles, OnAllBundleDownloadCompleted);
                }
                else
                {
                    //todo 不更新关闭程序
