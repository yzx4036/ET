...
namespace SEyesETEditor
{
    ...
    public class MonoBehaviourAdapterEditor: Editor
    {
            ...
            if (instance != null)
            {
                ...
                foreach (var i in instance.Type.FieldMapping)
                {
                    ...
                    if (cType.IsPrimitive) //如果是基础类型
                    {
                        ...
                        {
                            Log.Error($"无法序列化{name}，{e.Message}");
                            EditorGUILayout.LabelField(name, instance[i.Value].ToString());
                        }
                    }
                    else
                    {
                        ...
                        else if (cType == typeof (JsonData)) //可以折叠显示Json数据
                        {
                            if (instance[i.Value] != null)
                            {
                                ...
                                {
                                    instance[i.Value] = EditorGUILayout.TextArea(((JsonData) instance[i.Value]).ToString());
                                }
                        ...
                        else if (typeof (UnityEngine.Object).IsAssignableFrom(cType))
                        {
                            ...
                            //     try
                            //     {
                            //         var clrInstance = (MonoBehaviour) Tools.FindObjectsOfTypeAll(typeof(CrossBindingAdaptorType))
                            //             .Find(adaptor =>
                        ...
                        else if (type.ReflectionType.ToString().Contains("BindableProperty") && obj != null)
                        {
                            ...
                            {
                                //可更改
                                var data = ConvertSimpleType(EditorGUILayout.TextField(name, val.ToString()),
                                    genericType);
                                if (data != null) //尝试更改
                                {
        ...
        { 
            ...
            { 
                Log.Error("无法转换成类型：'" + value.ToString() + "' ==> " + destinationType); 
            } 
            try 
            { 
            ...
            {
                Log.Error("类型转换出错：'" + value.ToString() + "' ==> " + destinationType + "\n" + e.Message);
