        ...
        public static string GetPath(ILPath pathType)
        {
            ...
            switch (pathType)
            {
                ...
                case ILPath.MainAssemblyPath:
                    path = string.Empty;
                    for (int i = 0; i < asset.mainDllNameList.Count; i++)
                    {
                        path += asset.mainDllNameList[i];
                        if (i < asset.mainDllNameList.Count - 1)
                        {
                            path += ",";
                        }
                    }
                ...
                case ILPath.ILAssemblyPath:
                    path = string.Empty;
                    for (int i = 0; i < asset.ilrDllNameList.Count; i++)
                    {
                        path += asset.ilrDllNameList[i];
                        if (i < asset.ilrDllNameList.Count - 1)
                        {
                            path += ",";
                        }
                    }
        ...
        public static void SetPath(ILPath pathType, string newPath)
        {
            ...
            switch (pathType)
            {
                ...
                case ILPath.BindingOutputPath:
                    ...
                    break;
                case ILPath.MainAssemblyPath:
                    // asset.mainDllNameList = newPath;
                    break;
                case ILPath.ILAssemblyPath:
                    // asset.ilrDllNameList = newPath;
                    break;
                case ILPath.TemplatePath:
